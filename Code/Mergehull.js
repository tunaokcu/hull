...
function merge(h1, h2){
    if (h1.length + h2.length <= BASE_CASE){
        let merged = h1.concat(h2);
        merged = merged.sort( (a,b ) => comparePoints(a, b, centroid(merged)));
...
function mergeOutside(h1, h2, center1){
    ...
    for(  i = 0; i < h2.length; i++){
        if ( left(h2[i], h2[(i+1)%h2n], center1)  && !left( h2[(i-1+h2n)%h2n],h2[i], center1) ){
            minindex = i;
        }
        if ( !left(h2[i], h2[(i+1)%h2n], center1)  && left( h2[(i-1+h2n)%h2n],h2[i], center1) ){
            maxindex = i;
    ...
    {
        h2temp = h2.slice(minindex, h2n).concat(h2.slice(0, maxindex+1));
    }
    
    //console.log("h2temp: ", h2temp);
